...
- What R can do to help your projects
- How to run R code
...
Launch RStudio now if you have not done so already.
...
- R code can be run interactively both in the R console and RStudio. 
- Code is run one line or command at a time using `Enter`
- The selected line in an R source file open in RStudio can be run using `Ctrl+Enter` in interactive sessions
...
- Source files can be run in their entirety using the `source()` function, or in RStudio, using `Ctrl+Shift+S`
- Source files can be shared with partners to create repeatable results
...
R packages are installed in the console via the `install.packages()` function. This will install packages from CRAN, or local directories. Packages in development or not on CRAN are installed via the `install_github()` function, in the package `devtools`. Examples of installing packages are available inside this sample project at `./Code/R/Boot/Check Packages.R`.
...
R packages are loaded in the console via the `library()` function. This will load packages that have already been installed into the current R environment. Once a package is loaded, it is loaded for the remainder of the R session, and is not script dependent. Note that some packages have dependencies and may benefit from being loaded in a specific order. Examples of loading packages are available inside this sample project at `./Code/R/Boot/Check Packages.R`.

:::

::: {.column}
![List of Packages Available and Loaded via GUI in RStudio](./Assets/Images/LoadPackages.png)
:::
::::::
...
- The `Project Directory` or `Working Directory` is where R is currently running from
- By default, the `Working Directory` will be the project directory, but can be identified or changed using `getwd()` and `setwd()`
- `Project Directories` allow the use of relative file paths in code, and ease of version controlling or collaboration
...
- After R starts, it looks for a file named `.Rprofile` at the top level of the `Working Directory`, and executes the contents as an R script
- The `.Rprofile` is useful for having  consistent and project-specific environments and document rendering
- When R closes, it looks for a function in the environment named `.Last()`, and executes the function
- The `.Last()` is useful for generating time-stamps, log files, or removing any unwanted temporary files
...
Reference Materials Available via USB
